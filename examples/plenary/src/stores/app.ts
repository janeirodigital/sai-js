...
import { createSolidLdoDataset, changeData as ldoChangeData, commitData as ldoCommitData } from '@ldo/solid';
import { EventShapeType, OrganizationShapeType, PersonShapeType, RoleShapeType } from '../../ldo/Event$.shapeTypes';
import { Event, Role, Person, Organization } from '../../ldo/Event$.typings';
// FAKE data
...
export const useAppStore = defineStore('app', () => {
  ...
  async function createEvent(eventName: string, organizationId: string, podId: string, chair: Person, startDate: Date) {
  ...
  function formatDateTime(dateTime: string) {
    return new Date(dateTime).toLocaleString(navigator.language, { dateStyle: 'short', timeStyle: 'short' });
  }
  ...
  function isPresenceSet(event: Event) {
    ...
    return (
      ...
      //@ts-expect-error
      event.attendee.map((presence) => presence.agent['@id']).includes(user.value['@id']) ||
      //@ts-expect-error
      event.absentee.map((presence) => presence.agent['@id']).includes(user.value['@id'])
    );
  }
