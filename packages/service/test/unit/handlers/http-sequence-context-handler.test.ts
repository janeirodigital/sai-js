...
describe('HttpSequenceContextHandler', () => {
  ...
  test('executes context handlers in the same order as passed in the constructors', async () => {
    const firstHandler = {
      ...
      handle: vi.fn().mockImplementation((ctx: any) => of({ ...ctx, pristine: false, first: true }))
    };

    const secondHandler = {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      handle: vi.fn().mockImplementation((ctx: any) => of({ ...ctx, pristine: true, second: true }))
    };
  ...
  test('aborts if any handler throws', async () => {
    ...
    const thirdHandler = {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      handle: vi.fn().mockImplementation((ctx: any) => of({ ...ctx, pristine: false, third: true }))
    };
