...
describe('getters', () => {
  test('should provide hasDataInstance', async () => {
    const selectedDataAuthorizationIri = 'https://auth.alice.example/bee6bc10-2eb9-4b2d-b0c4-84c5d9039e53';
...
describe('generateSourceDataGrants', () => {
  ...
  test('should throw if called on authorization with Inherited scope', async () => {
    const inheritedDataAuthorizationIri = 'https://auth.alice.example/ecdf7b5e-5123-4a93-87bc-86ef6de389ff';
    const registrySet = await factory.crud.registrySet(registrySetIri);
    const granteeRegistration = await factory.crud.socialAgentRegistration(granteeRegistrationIri);
    const dataAuthorization = await factory.readable.dataAuthorization(inheritedDataAuthorizationIri);
    await expect(
      // @ts-ignore
      dataAuthorization.generateSourceDataGrants(registrySet.hasDataRegistry, granteeRegistration)
    ).rejects.toThrow('this method should not be callend on data authorizations with Inherited scope');
  });

  test('should generate source data grant for specific data registration', async () => {
    const allFromRegistryDataAuthorizationIri = 'https://auth.alice.example/d246d4da-79d9-4232-b5ab-94282cd0a63b';
    const registrySet = await factory.crud.registrySet(registrySetIri);
    const granteeRegistration = await factory.crud.socialAgentRegistration(granteeRegistrationIri);
    const dataAuthorization = await factory.readable.dataAuthorization(allFromRegistryDataAuthorizationIri);
    // @ts-ignore
  ...
  test('should generate source data grant for specific data instances', async () => {
    const selectedFromRegistryDataAuthorizationIri = 'https://auth.alice.example/8307e5b4-4fd6-4e76-99bf-64df6a7d2894';
    const registrySet = await factory.crud.registrySet(registrySetIri);
    const granteeRegistration = await factory.crud.socialAgentRegistration(granteeRegistrationIri);
    const dataAuthorization = await factory.readable.dataAuthorization(selectedFromRegistryDataAuthorizationIri);
    // @ts-ignore
    const dataGrants = await dataAuthorization.generateSourceDataGrants(
      registrySet.hasDataRegistry,
      granteeRegistration
...
describe('generateDelegatedDataGrants', () => {
  ...
  test('should throw if called on authorization with Inherited scope', async () => {
    const inheritedDataAuthorizationIri = 'https://auth.alice.example/6a9feb57-252b-43b2-8470-5a938888b2fa';
    ...
    await expect(
      // @ts-ignore
      dataAuthorization.generateDelegatedDataGrants(registrySet.hasAgentRegistry, granteeRegistration)
    ).rejects.toThrow('this method should not be callend on data authorizations with Inherited scope');
  });

  test('should not generate delegated data grant for data owner themselve', async () => {
    const allDataAuthorizationIri = 'https://auth.alice.example/9ac499ce-f1a2-44c2-a5fb-01e3eb9a5bc9';
    const registrySet = await factory.crud.registrySet(registrySetIri);
    const granteeRegistration = await factory.crud.socialAgentRegistration(granteeRegistrationIri);
  ...
  test('should generate source data grant for specific data registration', async () => {
    const allFromRegistryDataAuthorizationIri = 'https://auth.alice.example/dbad38a1-4f99-4cd3-9107-23743a4059a8';
    const registrySet = await factory.crud.registrySet(registrySetIri);
