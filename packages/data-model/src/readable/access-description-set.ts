...
export class ReadableAccessDescriptionSet extends ReadableResource {
  ...
  public static async build(iri: string, factory: AuthorizationAgentFactory): Promise<ReadableAccessDescriptionSet> {
    const instance = new ReadableAccessDescriptionSet(iri, factory);
    await instance.bootstrap();
    return instance;
  }

  public static findInLanguage(resource: ReadableResource, descriptionLang: string): string | undefined {
    // we can skip matching on INTEROP.hasAccessDescriptionSet since nothing else uses INTEROP.usesLanguage
    return resource.getQuad(null, INTEROP.usesLanguage, DataFactory.literal(descriptionLang, XSD.language))?.subject
      .value;
  }
}
